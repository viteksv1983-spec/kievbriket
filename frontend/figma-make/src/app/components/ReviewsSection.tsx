...
function StarRow({ count, bright }: { count: number; bright?: boolean }) {
  return (
...
function ReviewCard({ r, delay }: { r: typeof reviews[number]; delay: string }) {
...
export function ReviewsSection() {
  ...
  return (
    ...
    >
      {/* No separate top overlay needed — gradient bg already bridges from dark */}
      ...
      <style>{`
        ...
        .reviews-grid .card { opacity: 0; transform: translateY(28px); transition: opacity 0.55s ease, transform 0.55s ease, border-color 0.25s, box-shadow 0.25s; }
        .reviews-grid.visible .card:nth-child(1) { opacity:1; transform:none; transition-delay:0s; }
        .reviews-grid.visible .card:nth-child(2) { opacity:1; transform:none; transition-delay:0.1s; }
        .reviews-grid.visible .card:nth-child(3) { opacity:1; transform:none; transition-delay:0.2s; }
        .reviews-grid.visible .card:hover { transform: translateY(-6px) !important; }
      `}</style>
    </section>
  );
}